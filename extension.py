...
def depth_first(runner, maze, goal: Optional[Tuple[int, int]] = None) -> Tuple[str, List[Tuple[int, int]]]:
    action = ""
    passed_pos = [(runner['x'], runner['y'])]
    intersections = [] #a nested list with [intersection (x), intersection (y), action taken, number of remaining possible ways]
    
    if goal is None:
        dimensions = maze_module.get_dimensions(maze)
        goal = (dimensions[0] - 1, dimensions[1] - 1)

    while (runner['x'], runner['y']) != goal:
        walls = runner_module.sense_walls(runner, maze) #(Left, Front, Right)

        # Check if we are back at an intersection
        if intersections and (runner['x'], runner['y']) == (intersections[-1][0], intersections[-1][1]):
            if intersections[-1][3] > 0:
                if intersections[-1][2] == "LF":
                    runner_module.forward(runner)
                    action += "F"
                    intersections[-1][2] = "F"
                    intersections[-1][3] -= 1
                elif intersections[-1][2] == "F":
                    if not walls[0]:
                        runner_module.turn(runner, 'Left')
                        runner_module.forward(runner)
                        action += "LF"
                        intersections[-1][2] = "LF"
                        intersections[-1][3] -= 1
                    elif not walls[2]:
                        runner_module.turn(runner, 'Right')
                        runner_module.forward(runner)
                        action += "RF"
                        intersections[-1][2] = "RF"
                        intersections[-1][3] -= 1
            else:#Go back to the point before the intersection
                if intersections[-1][2] == "F":
                    if not walls[2]:
                        runner_module.turn(runner, 'Right')
                        runner_module.forward(runner)
                        action += "RF"
                        ...
                        runner_module.forward(runner)
                        action += "LF"
                elif intersections[-1][2] == "LF":
                    runner_module.turn(runner, 'Left')
                    runner_module.forward(runner)
                    action += "LF"
                elif intersections[-1][2] == "RF":
                    runner_module.turn(runner, 'Right')
                    runner_module.forward(runner)
                    action += "RF"
                intersections.pop(-1)
        ...
        # Normal case
            ...
            if sum(walls) < 2:
                intersections.append([runner['x'], runner['y']])
            if not walls[1]:
                runner_module.forward(runner)
                action += "F"
                if sum(walls) < 2:
                    intersections[-1].extend(["F", len(walls) - sum(walls) - 1])
            elif not walls[0]:
                runner_module.turn(runner, 'Left')
                runner_module.forward(runner)
                action += "LF"
                if sum(walls) < 2:
                    intersections[-1].extend(["LF", 1])
            elif not walls[2]:
                runner_module.turn(runner, 'Right')
                runner_module.forward(runner)
                action += "RF"
